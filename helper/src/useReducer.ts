...
export function useSideReducer(
  reducer: any,
  init: any,
  initFun: any,
  eq: any,
...
type SideCall<F> = (
  updateEffect: (level: number, effect: EmptyFun) => void,
  fun: SetValue<SetValue<F>>,
  set: SetValue<F>,
) => void
function defaultSideCall<F>(
  updateEffect: (level: number, effect: EmptyFun) => void,
  fun: SetValue<SetValue<F>>,
  set: SetValue<F>,
) {
...
function useBaseReducer(
    ...
    function set(action: any, effects: EffectArray = emptyArray as any) {
      reconcile(function (env) {
        ...
        if (sideCall) {
          ...
          if (fun) {
            //只能这样做!!!!
            sideCall(env.updateEffect, fun, set)
          }
        }
        effects.forEach((effect) => {
          env.updateEffect(effect[0], effect[1])
        })
        //这里,合并多次,如果动作被执行了
        if (!realEq(oldValue, newValue)) {
          value.set(env, newValue)
  ...
  const data = hook.value.get(envModel)
