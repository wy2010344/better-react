    ...
    children() {
      ...
      useDom("button", {
        onClick() {
          ...
          const cursor = store.openCursor(IDBKeyRange.lowerBound(3, false))
          cursor.onsuccess = (e) => {
            const result = cursor.result
            if (result) {
              console.log(result.value, "vvv")
              result.continue()
            }
          }
          cursor.onerror = (e) => {
...
function createDB(name: string, version = 1) {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onupgradeneeded = (a) => {
      ...
      try {
        const store = request.transaction?.objectStore("todo")
