...
export class AVar {
  ...
  equals(v: any) {
    return v == this || (v instanceof AVar && v.name == this.name)
  }
}
 ...
 * @returns 
 */
export function unify(a: AType, b: AType, sub: ASubsitution<AType>): [boolean, ASubsitution<AType>] {
  a = walk(a, sub)
  b = walk(b, sub)
  if (a == b) {
    return [true, sub]
...
const fail: AGoal<any> = function () {
  return null
}
type DelayStream<V> = () => AStream<V>
export const emptyDelayStream: DelayStream<any> = () => null
const success: AGoal<any> = function (sub) {
  return APair.of(sub, emptyDelayStream)
}
/**
 ...
 * @returns 
 */
  and<T>(a: AGoal<T>, b: AGoal<T>): AGoal<T> {
    return function (sub) {
      return streamBindGoal(a(sub), b)
    }
  },
  or<T>(a: AGoal<T>, b: AGoal<T>): AGoal<T> {
    return function (sub) {
