...
import { animateFrame, moveEdgeScroll, PagePoint, subscribeDragMove, subscribeMove, subscribeScroller, subscribeScrollerAll } from "wy-dom-helper";
    ...
    const container = dom.div({
      ...
      const outList = renderArrayToArray(orderList, v => v.index, function (row, index) {
        ...
        const div = renderRow(row, e => {
          ...
          const destroyScroll = subscribeScroller(container, 'y', m => {
            const info = moveInfo.get()
            if (info) {
              info.transY.slientDiff(m)
            }
          })
          moveInfo.set({
            key: row.index,
          ...
          const destroy = subscribeDragMove((e, p) => {
            const info = moveInfo.get()
            if (info && !info.finished) {
              if (e) {
