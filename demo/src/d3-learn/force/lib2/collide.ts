...
import { Direction, ForceNode, NodeForce } from "./reducerForce";
import { DIMType, TreeNode } from "../lib/model";
import { GetValue, emptyObject } from "wy-helper";
import jiggle from "../lib/jiggle";
...
function apply(
  ...
  var data = treeNode.data, rj = treeNode.r, r = g.ri + rj;
  if (data) {
    if (data.index > g.node.index) {
      var x = g.xi - data.x.d - data.x.v,
      ...
      if (l < r * r) {
        ...
        if (g.nDim > 1) { g.node.y.v += (y *= l) * r; }
        if (g.nDim > 2) { g.node.z.v += (z *= l) * r; }

        data.x.v -= x * (r = 1 - r);
        if (g.nDim > 1) { data.y.v -= y * r; }
        if (g.nDim > 2) { data.z.v -= z * r; }
      }
    }
    return;
  }
  return x0 > g.xi + r || x1 < g.xi - r
...
function prepare(treeNode: TreeNode<ForceNode<any>>,) {
  ...
  for (var i = treeNode.r = 0; i < Math.pow(2, g.nDim); ++i) {
