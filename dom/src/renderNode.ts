...
const domCreater: NodeMemoCreater<any, any, any> = (e) => {
  return new NodeHelper(
    document.createElement(e.trigger),
    mergeFDomAttr,
    emptyArray,
  );
};
...
function renderHelper(
  helper: NodeHelper<any, any>,
  args: FMergeChildAttr<any>,
) {
  ...
  if (args.childrenType == "html") {
    ...
    if (tp == "function") {
      ...
      (children as unknown as any)(helper.node);
      hookEndTempOps(before!);
    } else if (tp == "string" || tp == "number") {
      hookAttrEffect(() => {
        helper.updateContent("text", children as unknown as string);
      });
    }
  }
  return helper.node;
...
export type FSvgAttributes<T extends SvgElementType> = WithCenterMap<
  FSvgAttribute<T>
> &
  BSvgEvent<T> &
