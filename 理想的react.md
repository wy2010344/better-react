...
理想的纯粹无副作用
全局只有一个状态,像elm,这样比较纯粹.
因为分散的状态总面临状态同步问题.
但对于一些自封闭状态,如exitAnimate里面,通过更改version触发更新,再去将其绑定到全局上命名,似乎不值.
可以自己这样设计.
平时设计组件也要小心,尽可能状态与纯组件可分离.
在需要同步的地方没能实现状态与组件分离,就是失败的设计.

但相对于实时性mvr,变量似乎任何地方都可以修改
但希望收紧权限,只允许某些修改.
组件只有属性与事件,所以不应该接受RWValue.只有巧合需要设值与读值,可以简化封装,内部仍然是可以隔离开的.
